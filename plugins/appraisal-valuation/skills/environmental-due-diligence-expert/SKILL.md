...
See [regulatory-pathway.md](./regulatory-pathway.md) for full MOE process, three-outcome decision tree, timeline estimates, RSC requirements, and MOE Tier 1 GQS contaminant tables.
...
See [cleanup-cost-estimation.md](./cleanup-cost-estimation.md) for full cost component tables (excavation, capping, in-situ treatment, professional services, post-remediation), the six-step estimation process, volume calculations, conservative/expected/optimistic scenario framework, and risk-adjusted formulas with worked examples.
...
**Toolkit components**:
1. **Seller reps & warranties** in PSA (no contamination, no violations, prior ESAs disclosed, no spill history)
2. **Indemnity** — typical survival 3-7 yrs, cap $500K-$2M, threshold $10K-$25K
3. **Holdback/escrow** — 1-2% (clean Phase I) to 10-20% (significant Phase II exceedance), released on RSC filing or phased
4. **Insurance** — seller liability (pre-closing), buyer cost-cap (post-closing $250K-$1M coverage), pollution liability (operational)

See [liability-allocation-strategies.md](./liability-allocation-strategies.md) for full PSA representation language, indemnity term sheets, holdback calibration table, and insurance product details.
...
See [price-adjustment-examples.md](./price-adjustment-examples.md) for medium-risk (Tier 2), high-risk (severe contamination requiring seller remediation), service station Phase I review, manufacturing metals Phase II, TCE/chlorinated solvent Tier 2, HREC dry cleaner, and settlement/expropriation integration examples.
...
1. **Obtain documents**: Phase I/II ESAs, Risk Assessment, RSC, spill reports, regulatory database searches.
2. **Interpret Phase I**: Identify RECs (current, historical, controlled); flag data gaps.
3. **Analyze Phase II**: Contaminants, soil vs. groundwater, concentration vs. Tier 1 GQS, contaminated volume.
4. **Estimate cleanup cost**: Method selection → volume × unit costs + professional + monitoring → risk-adjustment → conservative/expected/optimistic.
5. **Regulatory timeline**: Non-exceedance (0 mo) / Tier 1 (6-12 mo) / Tier 2 (12-24 mo).
6. **Calculate price adjustment**: Risk-adjusted cost × discount rate + delay PV.
7. **Allocate liability**: Apply decision tree (indemnity / holdback / insurance / seller remediation).
...
- [cleanup-cost-estimation.md](./cleanup-cost-estimation.md) — Complete cost component tables, six-step estimation process, scenario modeling, risk-adjustment formulas
- [regulatory-pathway.md](./regulatory-pathway.md) — Full MOE approval process, three outcomes, timeline tables, RSC requirements/benefits/exceptions, Tier 1 GQS contaminant tables
- [liability-allocation-strategies.md](./liability-allocation-strategies.md) — PSA representation language, indemnity term sheets, holdback calibration, insurance product details
- [price-adjustment-examples.md](./price-adjustment-examples.md) — Low/medium/high risk worked examples, four extended use cases (service station, manufacturing, TCE solvent, dry cleaner HREC), settlement/expropriation integration
